"use strict";
(() => {
    let currentHosting = null;
    function initializeHostingDetailsPage() {
    ...
    async function loadHostingDetails() {
        ...
        const response = await typedWindow.UserPanelApi?.request(`/HostingAccounts/${id}/details`, { method: 'GET' }, true);
    ...
    function renderHostingSummary(item) {
    ...
    function renderHostingResources(item) {
    ...
    async function updateHostingStatus(status) {
        if (!currentHosting) {
            return;
        }
        const typedWindow = window;
        const response = await typedWindow.UserPanelApi?.request(`/HostingAccounts/${currentHosting.id}`, {
            method: 'PUT',
    ...
    async function updateHostingPlan(direction) {
        if (!currentHosting) {
            return;
        }
        const typedWindow = window;
    ...
    function formatHostingDetailsDate(value) {
    ...
    function escapeHostingDetailsText(value) {
        return value
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    function setupHostingDetailsObserver() {
...
//# sourceMappingURL=hosting-details.js.map