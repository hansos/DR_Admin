interface CheckoutSuccessOrderMarker {
...
interface CheckoutSuccessCartState {
    clear: () => void;
}

interface CheckoutSuccessWindow extends Window {
    UserPanelCart?: CheckoutSuccessCartState;
}

const checkoutSuccessOrderMarkerStorageKey = 'up_checkout_last_added_order';

function initializeCheckoutSuccessPage(): void {
    ...
    if (status === 'success') {
        const typedWindow = window as CheckoutSuccessWindow;
        typedWindow.UserPanelCart?.clear();
    }
}

function getCheckoutSuccessStoredOrderMarker(): CheckoutSuccessOrderMarker | null {
    try {
        ...
        return JSON.parse(raw) as CheckoutSuccessOrderMarker;
    } catch {
        return null;
    }
}

function renderCheckoutSuccessStatus(status: string): void {
    ...
    const primaryLink = document.getElementById('checkout-success-primary-link') as HTMLAnchorElement | null;
...
function renderCheckoutSuccessOrders(marker: CheckoutSuccessOrderMarker | null, query: URLSearchParams): void {
...
function escapeCheckoutSuccessText(value: string): string {
    return value
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
