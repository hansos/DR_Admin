...
interface SecuritySessionsWindow extends Window {
    UserPanelApi?: {
...
function initializeSecuritySessionsPage(): void {
...
async function loadSecuritySessions(): Promise<void> {
    const typedWindow = window as SecuritySessionsWindow;
    typedWindow.UserPanelAlerts?.hide('security-sessions-alert-error');

    const response = await typedWindow.UserPanelApi?.request<PagedResult<LoginHistoryDto> | LoginHistoryDto[]>('/LoginHistories?pageNumber=1&pageSize=50', { method: 'GET' }, true);
...
function normalizeSecuritySessions(payload: PagedResult<LoginHistoryDto> | LoginHistoryDto[]): LoginHistoryDto[] {
...
function renderSecuritySessions(items: LoginHistoryDto[]): void {
...
function formatSecuritySessionsDate(value: string): string {
...
function escapeSecuritySessionsText(value: string): string {
    return value
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
function setupSecuritySessionsObserver(): void {
