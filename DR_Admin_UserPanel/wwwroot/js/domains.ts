...
interface DomainsWindow extends Window {
    UserPanelApi?: {
...
function applyDomainsFilters(items: DomainDto[]): DomainDto[] {
...
function renderDomainsRows(items: DomainDto[]): void {
...
function setDomainsPaginationInfo(count: number): void {
...
function readDomainsInputValue(id: string): string {
    const input = document.getElementById(id) as HTMLInputElement | HTMLSelectElement | null;
    return input?.value.trim() ?? '';
}

function escapeDomainsText(value: string): string {
    return value
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
function setupDomainsObserver(): void {
