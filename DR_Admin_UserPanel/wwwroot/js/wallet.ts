...
function initializeWalletPage(): void {
...
async function loadWalletData(): Promise<void> {
    const typedWindow = window as WalletWindow;
    ...
    const [creditResponse, transactionsResponse] = await Promise.all([
        typedWindow.UserPanelApi?.request<CustomerCreditDto>(`/CustomerCredits/customer/${customerId}`, { method: 'GET' }, true),
        typedWindow.UserPanelApi?.request<CreditTransactionDto[]>(`/CustomerCredits/customer/${customerId}/transactions`, { method: 'GET' }, true)
    ]);
...
async function getWalletCustomerId(): Promise<number | null> {
    const typedWindow = window as WalletWindow;
...
function renderWalletBalance(data: CustomerCreditDto | null): void {
...
function renderWalletTransactions(items: CreditTransactionDto[]): void {
...
function escapeWalletText(value: string): string {
    return value
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
...
function setupWalletObserver(): void {
