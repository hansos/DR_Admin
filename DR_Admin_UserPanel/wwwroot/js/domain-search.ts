...
interface CalculatePricingRequestDto {
    tldId: number;
    operationType: 'Registration' | 'Renewal' | 'Transfer';
    years: number;
    isFirstYear: boolean;
}

interface CalculatePricingResponseDto {
    finalPrice?: number;
    FinalPrice?: number;
    currency?: string;
    Currency?: string;
...
let serviceCatalogRequest: Promise<ServiceDto[] | null> | null = null;
let hostingCatalog: HostingPackageDto[] | null = null;
...
function updateDomainSearchDeleteOrderVisibility(): void {
    ...
    const backToSearchButton = document.getElementById('domain-search-back-to-search') as HTMLButtonElement | null;
    ...
    serviceList?.addEventListener('click', (event: Event) => {
        const target = event.target as HTMLElement;
        const button = target.closest<HTMLButtonElement>('[data-service-id]');
        if (!button) {
            return;
        }
    ...
    hostingList?.addEventListener('click', (event: Event) => {
        const target = event.target as HTMLElement;
        const button = target.closest<HTMLButtonElement>('[data-hosting-id]');
        if (!button) {
            return;
        }
        ...
        const container = button.closest<HTMLElement>('[data-hosting-row]');
        const cycleSelect = container?.querySelector<HTMLSelectElement>('[data-hosting-cycle]');
        const billingCycle = cycleSelect?.value === 'yearly' ? 'yearly' : 'monthly';

        toggleHostingSelection(id, billingCycle);
    });

    hostingList?.addEventListener('change', (event: Event) => {
        const target = event.target as HTMLElement;
        const cycleSelect = target.closest<HTMLSelectElement>('[data-hosting-cycle]');
        if (!cycleSelect) {
            return;
        }
...
function getSelectedDomainPrice(result: DomainAvailabilityResult): number {
...
function getAddAndBundleButtonLabel(result: DomainAvailabilityResult): string {
    const typedWindow = window as DomainSearchWindow;
...
function renderSelectedDomainSummary(selection: { domainName: string; periodYears: number; price: string } | null): void {
...
function updateFlowProgressIndicator(): void {
    ...
    const applyState = (indicator: HTMLElement, label: HTMLElement, state: 'pending' | 'active' | 'completed'): void => {
    ...
    applyState(step1Indicator as HTMLElement, step1Label as HTMLElement, step > 1 ? 'completed' : step === 1 ? 'active' : 'pending');
    applyState(step2Indicator as HTMLElement, step2Label as HTMLElement, step > 2 ? 'completed' : step === 2 ? 'active' : 'pending');
    applyState(step3Indicator as HTMLElement, step3Label as HTMLElement, step === 3 ? 'active' : 'pending');
}
...
function setDomainFormLocked(isLocked: boolean): void {
    ...
    const controls = Array.from(form.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLButtonElement>('input, select, button'));
...
function switchToDomainSearchMode(): void {
    isDomainSelectionLocked = false;
    setDomainFormLocked(false);
    setUpsellVisibility(false);
    renderResult(latestResult);
...
async function getDomainRegistrationPrice(domainName: string, years: number): Promise<number | null> {
    const tldExtension = getTldExtension(domainName);
    if (!tldExtension) {
    ...
    const pricingRequest: CalculatePricingRequestDto = {
    ...
    const pricingResponse = await typedWindow.UserPanelApi?.request<CalculatePricingResponseDto>('/tld-pricing/calculate', {
        method: 'POST',
        headers: {
...
function getTldExtension(domainName: string): string | null {
...
function setUpsellVisibility(isVisible: boolean): void {
...
async function renderUpsellOptions(): Promise<void> {
    await Promise.all([
        renderServiceUpsell(),
        renderHostingUpsell()
    ]);
...
async function renderServiceUpsell(): Promise<void> {
    ...
    list.innerHTML = catalog.map((item: ServiceDto) => {
        const isSelected = selectedServices.some((entry: CartServiceItem) => entry.id === item.id);
...
async function renderHostingUpsell(): Promise<void> {
    ...
    list.innerHTML = catalog.map((item: HostingPackageDto) => {
        const selected = selectedHosting.find((entry: CartHostingItem) => entry.id === item.id);
...
async function getServiceCatalog(): Promise<ServiceDto[] | null> {
    if (serviceCatalog) {
...
function toggleServiceSelection(serviceId: number): void {
    const typedWindow = window as DomainSearchWindow;
    const state = typedWindow.UserPanelCart?.getState();
    if (!state) {
        return;
    ...
    const currentIndex = state.services.findIndex((item: CartServiceItem) => item.id === serviceId);
    if (currentIndex >= 0) {
    ...
    const source = serviceCatalog?.find((item: ServiceDto) => item.id === serviceId);
    if (!source) {
        return;
    }
...
async function getHostingCatalog(): Promise<HostingPackageDto[] | null> {
    if (hostingCatalog) {
...
function toggleHostingSelection(hostingId: number, billingCycle: 'monthly' | 'yearly'): void {
    const typedWindow = window as DomainSearchWindow;
    const state = typedWindow.UserPanelCart?.getState();
    if (!state) {
        return;
    ...
    const existingIndex = state.hosting.findIndex((item: CartHostingItem) => item.id === hostingId);
    if (existingIndex >= 0) {
    ...
    const source = hostingCatalog?.find((item: HostingPackageDto) => item.id === hostingId);
    if (!source) {
        return;
    }
...
function updateHostingCycle(hostingId: number, billingCycle: 'monthly' | 'yearly'): void {
    const typedWindow = window as DomainSearchWindow;
    const state = typedWindow.UserPanelCart?.getState();
    if (!state) {
        return;
    ...
    const existing = state.hosting.find((item: CartHostingItem) => item.id === hostingId);
    if (!existing) {
        return;
    }
...
async function getDefaultRegistrarSelection(): Promise<DefaultRegistrarSelection | null> {
    ...
    defaultRegistrarCodeRequest = (async () => {
        ...
        const registrar = defaultRegistrarCode
            ? response.data.find((item: RegistrarLookupDto) => item.code === defaultRegistrarCode) ?? response.data.find((item: RegistrarLookupDto) => item.isDefault)
            : response.data.find((item: RegistrarLookupDto) => item.isDefault);
