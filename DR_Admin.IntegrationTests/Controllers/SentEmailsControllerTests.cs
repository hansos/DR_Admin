...
public class SentEmailsControllerTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    [Fact]
    [Trait("Category", "SentEmails")]
    public async Task GetAllSentEmails_WithAdminRole_ReturnsOk()
    {
        // Arrange
        await SeedSentEmails();
        var token = await GetAdminTokenAsync();
        _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

        // Act
        var response = await _client.GetAsync("/api/v1/SentEmails", TestContext.Current.CancellationToken);
    ...
    [Fact]
    [Trait("Category", "SentEmails")]
    public async Task GetAllSentEmails_WithoutAuthentication_ReturnsUnauthorized()
    {
        // Act
        var response = await _client.GetAsync("/api/v1/SentEmails", TestContext.Current.CancellationToken);
    ...
    private async Task<int> SeedSentEmails()
    {
        ...
        await context.SentEmails.AddAsync(email);
