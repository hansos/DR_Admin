...
public class ServerControlPanelsControllerTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    [Fact]
    [Trait("Category", "ServerControlPanels")]
    public async Task GetAllServerControlPanels_WithAdminRole_ReturnsOk()
    {
        // Arrange
        await SeedServerControlPanels();
    ...
    private async Task<int> SeedServerControlPanels()
    {
        ...
        await context.ControlPanelTypes.AddAsync(controlPanelType);
        await context.SaveChangesAsync();

        var serverControlPanel = new ServerControlPanel
        {
            ServerId = server.Id,
            ControlPanelTypeId = controlPanelType.Id,
            Username = "admin",
            PasswordHash = "password",
            ApiUrl = "https://panel.test.com",
            Status = "Active",
            CreatedAt = DateTime.UtcNow,
            UpdatedAt = DateTime.UtcNow
        };
        await context.ServerControlPanels.AddAsync(serverControlPanel);
        await context.SaveChangesAsync();

        return serverControlPanel.Id;
    }
