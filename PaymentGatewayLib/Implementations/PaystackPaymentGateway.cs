...
namespace PaymentGatewayLib.Implementations
{
    public class PaystackPaymentGateway : BasePaymentGateway, IPaymentGateway
    {
        ...
        public PaystackPaymentGateway(string publicKey, string secretKey, bool useTestMode = true) { _logger = Log.ForContext<PaystackPaymentGateway>(); _publicKey = publicKey ?? throw new ArgumentNullException(nameof(publicKey)); _secretKey = secretKey ?? throw new ArgumentNullException(nameof(secretKey)); }
