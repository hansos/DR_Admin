...
namespace PaymentGatewayLib.Implementations
{
    public class PayUPaymentGateway : BasePaymentGateway, IPaymentGateway
    {
        ...
        public PayUPaymentGateway(string posId, string clientSecret, bool useSandbox = true) { _logger = Log.ForContext<PayUPaymentGateway>(); _posId = posId ?? throw new ArgumentNullException(nameof(posId)); _clientSecret = clientSecret ?? throw new ArgumentNullException(nameof(clientSecret)); }
