...
namespace PaymentGatewayLib.Implementations
{
    public class KlarnaPaymentGateway : BasePaymentGateway, IPaymentGateway
    {
        ...
        public KlarnaPaymentGateway(string username, string password, bool useTestMode = true) { _logger = Log.ForContext<KlarnaPaymentGateway>(); _username = username ?? throw new ArgumentNullException(nameof(username)); _password = password ?? throw new ArgumentNullException(nameof(password)); }
