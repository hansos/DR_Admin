(() => {
    ...
    let allRegistrations: TaxRegistration[] = [];
    ...
    function normalizeRegistration(item: Record<string, unknown>): TaxRegistration {
        return {
    ...
    function normalizeJurisdiction(item: Record<string, unknown>): TaxJurisdiction {
        return {
            id: Number(item.id ?? item.Id ?? 0),
    ...
    function renderRows(): void {
        ...
        const jurisdictionMap = new Map<number, string>(allJurisdictions.map((x) => [x.id, `${x.code} - ${x.name}`]));
    ...
    function renderJurisdictionOptions(): void {
        const select = document.getElementById('tax-registrations-jurisdiction') as HTMLSelectElement | null;
        if (!select) {
            return;
        }
    ...
    function openCreate(): void {
        ...
        (document.getElementById('tax-registrations-form') as HTMLFormElement | null)?.reset();
    ...
    function openEdit(id: number): void {
        ...
        const active = document.getElementById('tax-registrations-is-active') as HTMLInputElement | null;
        if (active) {
