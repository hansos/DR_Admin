...
async function loadDomainHistory(): Promise<void> {
    ...
    const response = await apiRequest<DomainHistoryEntry[]>(`${getApiBaseUrl()}/RegisteredDomainHistories/domain/${domainId}`, { method: 'GET' });
    if (!response.success) {
        loading?.classList.add('d-none');
        empty?.classList.remove('d-none');
        setHistoryCount(0);
...
function renderDomainHistory(entries: DomainHistoryEntry[]): void {
    const loading = document.getElementById('domain-history-loading');
    const content = document.getElementById('domain-history-content');
    const empty = document.getElementById('domain-history-empty');
...
function getHistoryActionTypeLabel(actionType: number): string {
...
function getHistoryActionTypeBadgeClass(actionType: number): string {
...
function setHistoryCount(count: number): void {
