...
function getApiBaseUrl(): string {
    const appSettings = (window as unknown as { AppSettings?: { apiBaseUrl?: string } }).AppSettings;
    return appSettings?.apiBaseUrl ?? '';
}

function getAuthToken(): string | null {
    const auth = (window as unknown as { Auth?: { getToken?: () => string | null } }).Auth;
    if (auth?.getToken) {
...
async function apiRequest<T>(url: string, options: RequestInit): Promise<RequestResult<T>> {
    try {
...
function mapCompany(data: unknown): MyCompanyDto | null {
...
function toString(...values: unknown[]): string | null {
...
function fillForm(company: MyCompanyDto): void {
...
function collectPayload(): UpsertMyCompanyDto {
    return {
...
async function loadCompany(): Promise<void> {
    const response = await apiRequest<unknown>(`${getApiBaseUrl()}/MyCompany`, { method: 'GET' });
...
async function saveCompany(): Promise<void> {
    ...
    const response = await apiRequest<unknown>(`${getApiBaseUrl()}/MyCompany`, {
        method: 'PUT',
...
function initializeCompanyPage(): void {
