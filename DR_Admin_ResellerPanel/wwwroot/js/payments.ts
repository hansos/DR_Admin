(() => {
    ...
    let filteredPayments: PaymentTransaction[] = [];
    ...
    const extractPayments = (payload: unknown): unknown[] => {
    ...
    const normalizeAllocation = (item: unknown): PaymentTransactionAllocation => {
    ...
    const normalizePayment = (item: unknown): PaymentTransaction => {
        const row = (item ?? {}) as Record<string, unknown>;
    ...
    const renderAllocationsPanel = (payment: PaymentTransaction): string => {
    ...
    const loadPageSizeFromUi = (): void => {
        const select = document.getElementById('payments-page-size') as HTMLSelectElement | null;
    ...
    const resetFilters = (): void => {
        const status = document.getElementById('payments-filter-status') as HTMLSelectElement | null;
        const customerId = document.getElementById('payments-filter-customerid') as HTMLInputElement | null;
        const invoiceId = document.getElementById('payments-filter-invoiceid') as HTMLInputElement | null;
    ...
    const getPagedPayments = (): PaymentTransaction[] => {
    ...
    const loadPayments = async (): Promise<void> => {
        ...
        hideError();
        const response = await apiRequest<unknown>(`${getApiBaseUrl()}/PaymentTransactions`, { method: 'GET' });
    ...
    const bindEvents = (): void => {
        ...
        document.getElementById('payments-table-body')?.addEventListener('click', (event) => {
            const target = event.target as HTMLElement;
            const row = target.closest<HTMLTableRowElement>('tr[data-payment-id]');
            if (!row) {
                return;
            }
    ...
    const initializePage = (): void => {
        const page = document.getElementById('payments-page') as HTMLElement | null;
