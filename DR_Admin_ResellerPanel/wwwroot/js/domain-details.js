...
(function () {
    ...
    async function loadCustomerDomains() {
        ...
        const response = await apiRequest(`${getApiBaseUrl()}/RegisteredDomains/customer/${currentDomain.customerId}`, { method: 'GET' });
        if (!response.success) {
            loading?.classList.add('d-none');
            empty?.classList.remove('d-none');
            setCustomerDomainsCount(0);
    ...
    function renderCustomerDomains(domains) {
        const loading = document.getElementById('domain-customer-domains-loading');
        const content = document.getElementById('domain-customer-domains-content');
        const empty = document.getElementById('domain-customer-domains-empty');
    ...
    function setCustomerDomainsCount(count) {
