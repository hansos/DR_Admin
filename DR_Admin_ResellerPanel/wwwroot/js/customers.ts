...
async function applyCustomerToEditModal(customer: Customer, id: number): Promise<void> {
    await ensureCustomerEditorOptionsLoaded();
...
async function openEditById(id: number): Promise<void> {
    let customer = allCustomers.find((c) => c.id === id);

    if (!customer) {
        const response = await apiRequest<Customer>(`${getApiBaseUrl()}/Customers/${id}`, { method: 'GET' });
...
async function ensureCustomerEditorOptionsLoaded(): Promise<void> {
...
function initializeCustomerModal(): void {
    ...
    document.addEventListener('customers:open-edit', (event) => {
        const id = Number((event as CustomEvent)?.detail?.id ?? 0);
