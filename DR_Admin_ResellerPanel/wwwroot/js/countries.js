...
(() => {
    ...
    const loadCountries = async () => {
        ...
        const response = await apiRequest(`${getApiBaseUrl()}/Countries`, { method: 'GET' });
        if (!response.success) {
    ...
    const openCreate = () => {
        ...
        document.getElementById('countries-form')?.reset();
        const active = document.getElementById('countries-is-active');
        if (active) {
            active.checked = true;
        }
    ...
    const saveCountry = async () => {
        ...
        const isActive = document.getElementById('countries-is-active')?.checked ?? false;
        ...
        const response = editingId
            ? await apiRequest(`${getApiBaseUrl()}/Countries/${editingId}`, { method: 'PUT', body: JSON.stringify(payload) })
            : await apiRequest(`${getApiBaseUrl()}/Countries`, { method: 'POST', body: JSON.stringify(payload) });
        if (!response.success) {
    ...
    const doDelete = async () => {
        ...
        const response = await apiRequest(`${getApiBaseUrl()}/Countries/${pendingDeleteId}`, { method: 'DELETE' });
    ...
    const initializePage = async () => {
        const page = document.getElementById('countries-page');
