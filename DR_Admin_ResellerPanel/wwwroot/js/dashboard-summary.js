...
(function () {
    ...
    async function loadRecordStatsSummary() {
    ...
    function calculateRecordStats(items) {
        const now = Date.now();
    ...
    async function loadAllCustomersForStats() {
        const allItems = [];
        let pageNumber = 1;
        const pageSize = 200;
        let totalPages = 1;
    ...
    function renderRecordStatsSummary(prefix, summary) {
    ...
    function setRecordStatsUnavailable(prefix) {
    ...
    function setRecordStatsLoading(prefix, isLoading) {
        if (isLoading) {
            setText(`dashboard-summary-${prefix}-today`, '...');
            setText(`dashboard-summary-${prefix}-last24h`, '...');
            setText(`dashboard-summary-${prefix}-lastWeek`, '...');
