(() => {
    ...
    const extractInvoices = (payload: unknown): unknown[] => {
        ...
        const nestedData = objectPayload?.data as { items?: unknown; Items?: unknown; data?: unknown; Data?: unknown } | undefined;
    ...
    const normalizeInvoiceLine = (item: unknown): InvoiceLine => {
    ...
    const normalizeInvoice = (item: unknown): Invoice => {
        const row = (item ?? {}) as Record<string, unknown>;
    ...
    const renderInvoiceLinesPanel = (invoice: Invoice): string => {
    ...
    const loadPageSizeFromUi = (): void => {
        const select = document.getElementById('invoices-page-size') as HTMLSelectElement | null;
    ...
    const resetFilters = (): void => {
        const status = document.getElementById('invoices-filter-status') as HTMLSelectElement | null;
        const customerId = document.getElementById('invoices-filter-customerid') as HTMLInputElement | null;
    ...
    const getPagedInvoices = (): Invoice[] => {
    ...
    const loadInvoices = async (): Promise<void> => {
        ...
        hideError();
        const response = await apiRequest<unknown>(`${getApiBaseUrl()}/Invoices`, { method: 'GET' });
    ...
    const initializePage = (): void => {
        const page = document.getElementById('invoices-page') as HTMLElement | null;
